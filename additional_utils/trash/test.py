...
def get_vebinars():
    ...
    for userID in [19417123, 19417159]:
        for event in requests.get(
            f"https://userapi.webinar.ru/v3/users/{userID}/events/schedule",
            params=body,
            headers=HEADERS,
        ).json():
            for event in event["eventSessions"]:
                yield {
                    "startAt": str(event["startsAt"]),
                    "endAt": None,
...
def deleter(data: Iterable[Dict[str, str]]):
    for event in data:
        requests.delete(
            url=f'https://userapi.webinar.ru/v3/organization/events/{event["eventId"]}'
        )
...
def writer(data: Iterable[Dict[str, str]]):
    with open(
        ...
        dict_writer = csv.DictWriter(
            csvfile,
            fieldnames=["startAt", "endAt", "subject", "eventSessions", "eventId"],
        )
