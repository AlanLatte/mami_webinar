...
def worker(data):
    ...
    for webinar in data:
        ...
        answer = requests.put(url=f'https://userapi.webinar.ru/v3/organization/events/{str(webinar["eventId"])}/move', data=body, headers=HEADERS)
        print(answer)
        id += 1
        if id == len(ROOMS):
            id = 0
...
def reader():
    with open(os.path.join(INPUT_DIR_PATH, "svodniy_table.csv"), mode="r", encoding="utf-8") as csvfile:
