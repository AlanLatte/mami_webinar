...
def create_virtual_table(service):
    spreadsheet = service.spreadsheets().create(body = {
        'properties': {'title': 'Сие есть название документа1', 'locale': 'ru_RU'},
...
def create_new_sheet_in_table(info: list, spreadsheetID='1e_DqDCwrc3xlTWsXDB-zUuchWSWH_HA6wzogKbXPStE',):
    request_body, sheet_name = prepair_data_to_table(info)
    spreadsheet = service.spreadsheets().get(spreadsheetId = spreadsheetID).execute()
    append_list = service.spreadsheets().batchUpdate(spreadsheetId =\
    spreadsheet['spreadsheetId'], body =
    {
      "requests": [
        {
                "addSheet":
    ...
    print(request_body)
    append_info = service.spreadsheets().values().batchUpdate(spreadsheetId = spreadsheetID, body = {
        "valueInputOption": "USER_ENTERED",
        "data": request_body
    }).execute()

def prepair_data_to_table(info: list):
    for row in info:
...
def get_free_sheet_id(spreadsheet):
    mass = [i['properties']['sheetId'] for i in spreadsheet['sheets']]
    return(max(mass)+1)
